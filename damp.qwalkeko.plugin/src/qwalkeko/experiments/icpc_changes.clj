...
(def left
  (first
    (l/qwalkeko 1 [?left]
      (qwal/qwal a-graph pre-refactored-version pre-refactored-version []
        (l/in-source-code [curr]
          (logic/fresh [?typedecl ?tname]
            (jdt/ast :CompilationUnit ?left)
            (ast/compilationunit-typedeclaration|main ?left ?typedecl)
            (jdt/has :name ?typedecl ?tname)
            (jdt/name|simple-string ?tname "WeblogicDeploymentTool")))))))

(def right
  (first
    (l/qwalkeko 1 [?left]
      (qwal/qwal a-graph refactored-version refactored-version []
        (l/in-source-code [curr]
          (logic/fresh [?typedecl ?tname]
            (jdt/ast :CompilationUnit ?left)
            (ast/compilationunit-typedeclaration|main ?left ?typedecl)
